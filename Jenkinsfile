    ...
    withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
        stage('Deploye Code') {
			...
			if (isUnix()) {
			   ...
			   rmsg = bat returnStdout: true, script: "${toolbelt} force:source:deploy -x manifest/package/package.xml --postdestructivechanges manifest/destructiveChanges/destructiveChanges.xml"
			}
